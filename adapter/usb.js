 ...
 */
function USB(vid, pid, options){
  EventEmitter.call(this);
  var self = this;
  this.device = null;
  if(vid && pid) {
    this.device = USB.findByIds(vid, pid, options);
  }else if(vid){
...
USB.findByIds = function(vid, pid, options) {
    var devices = usb.getDeviceList();

    for (var i = 0; i < devices.length; i++) {
        var deviceDesc = devices[i].deviceDescriptor;
        if (Boolean(options)) {
            if (devices[i].busNumber == options.busNumber && devices[i].deviceAddress == options.deviceAddress) {
                return devices[i]
            } else {
                continue
            }
        } else if (deviceDesc.idVendor == vid && deviceDesc.idProduct == pid) {
            return devices[i]
        }
    }
}
...
USB.prototype.close = function(callback){
  ...
  this.device && this.device.close(callback);
  callback && callback(null, device);
  return this;
};
